...
function renderTable(tableLines) {
  ...
  if (coverIndex !== -1) {
    rows.forEach((row) => {
      if (!row[coverIndex]) {
        row[coverIndex] = "![No cover available](/profile/assets/images/no-cover.svg)";
      }
    });
  }
  ...
  return entries
    .map(
      ({ slug, title, hasTitle, created }) =>
        `<li><a href="/profile/${section.name}/${slug}">${escapeHtml(
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="/profile/assets/css/main.css" />
    <script src="/profile/assets/js/theme-toggle.js"></script>
    <script src="/profile/assets/js/copy-code.js"></script>
    <script src="/profile/assets/js/table-view.js"></script>
  </head>
  <body>
    <!-- include:header -->
    <main class="content">
...
<html lang="en">
  ...
  <body>
    <!-- include:header -->
    <main class="content">
      <link rel="stylesheet" href="/profile/assets/css/log.css" />
      <article class="log-entry">
        <div class="log-content">
          <a href="/profile/${section.name}" class="log-back">&larr; ${
            section.backLabel
          }</a>
          ${content}
        </div>
...
const PAGES = [
  {
    name: "shelves/books",
    src: join(ROOT, "src/shelves/books.md"),
    dist: join(ROOT, "dist/shelves/books"),
  },
  {
    name: "shelves/music",
    src: join(ROOT, "src/shelves/music.md"),
    dist: join(ROOT, "dist/shelves/music"),
  },
  {
    name: "shelves/games",
    src: join(ROOT, "src/shelves/games.md"),
    dist: join(ROOT, "dist/shelves/games"),
  },
];

const SHELVES_LINKS = [
  { href: "/profile/shelves/books", label: "Books" },
  { href: "/profile/shelves/music", label: "Music" },
  { href: "/profile/shelves/games", label: "Games" },
];

mkdirSync(join(ROOT, "dist/shelves"), { recursive: true });
writeFileSync(
  join(ROOT, "dist/shelves/index.html"),
  renderStandalonePage({
    title: "Shelves",
    description: "Books, music, and games I own.",
    content: `<ul>\n${SHELVES_LINKS.map(
      ({ href, label }) => `  <li><a href="${href}">${label}</a></li>`,
    ).join("\n")}\n</ul>`,
  }),
);
console.info("  built: shelves/index.html");
