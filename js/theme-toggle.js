function generateNewTheme() {
    ...
    if (mode === "light") {
        ...
        colors = {
            "--background": `hsl(${hue}, ${Math.min(baseSat * 0.4, 25)}%, ${bgLightness}%)`,
    ...
    if (themeType === 'vibrant') {
        colors["--highlight"] = `hsl(${complementaryHue}, ${85 + Math.random() * 15}%, ${mode === 'light' ? 45 : 65}%)`;
...
let randomThemeButtonRevealed = false;
...
function showRandomThemeButton() {
    ...
    randomThemeButtonRevealed = true;
}
